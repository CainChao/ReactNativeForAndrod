# ExampleInterface.java是一个实现ReactContextBaseJavaModule 接口文件，是主要负责实现React-Native(RN)和原生通信
	...
	如果是从原生传值到RN使用mContext.getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class)
                .emit("AndroidToRNMessage",mMessage);进行传值，在RN使用DeviceEventEmitter.addListener('AndroidToRNMessage',listener)进行取值
